...
<head>
	...
	<script>
	// This page only
	function expandToggleTap02(id){
		if($("#hidden2A"+id).css("display") == "table-row"){
			$("#hidden2A"+id).css("display", "none");
		}
		else{
			$("#hidden2A"+id).css("display", "table-row");
		}
		if($("#hidden2B"+id).css("display") == "table-row"){
			$("#hidden2B"+id).css("display", "none");
		}
		else{
			$("#hidden2B"+id).css("display", "table-row");
		}
	}
	</script>
</head>
<body>
	<div class="adminProduct_cardContainer adminProduct_productContainer">
		<table class="adminOrder_table">
			...
			<tbody>
				<c:forEach items="${currDeliveryLogs}" var="row" varStatus="idx">
					<tr class="adminProduct_listItem" onclick="expandToggleTap02(${row.delivery_id});" style="cursor: pointer">
					...
					<tr id="hidden2A${row.delivery_id}" class="adminOrder_additionalArea" style="font-weight: 700; border-top: 1px solid #bbb">
						<td>배송 위치</td>
						<td>배송 출발일</td>
						<td>배송 완료일</td>
						<td>고객 연락처</td>
					...
					<tr id="hidden2B${row.delivery_id}" class="adminOrder_additionalArea" style="border-bottom: 1px solid #bbb;">
						<c:choose>
				...
				<c:set var="listCnt" value="${currDeliveryLogs.size()}"/>
				<c:choose>
