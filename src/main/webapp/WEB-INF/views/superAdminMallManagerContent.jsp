...
<head>
	...
	function superAdmin_simpleUpdate(){
		...
		simpleUpdate(
				"superAdminManager_simpleUpdateForm", 
				"superAdminManager_simpleUpdate",
				"adminProduct_content");
	}
	</script>
</head>
<body>
	<form id="superAdminManager_simpleUpdateForm" onsubmit="return superAdmin_simpleUpdate()">
	<div class="adminProduct_cardContainer productStatus_extend">
		<p class="adminProduct_statusText">선택 관리자를 </p>
		...
		<input type="hidden" name="superAdminManager_uids[]" value="-1"/>
			...
			<tbody>
				<c:forEach items="${userVOs}" var="row" varStatus="idx">
					<tr class="adminProduct_listItem">
						<td>
							<input type="checkbox" name="superAdminManager_uids[]" 
								value="${row.u_id}" 
								class="adminProduct_itemCheckbox">
						</td>
						<td>${row.user_id}</td>
						<td>${row.mall_name}</td>
						<td class="adminProduct_itemName" onclick="location.href='superAdminDetailAdmin?u_id=${row.u_id}'">${row.user_name}</td>
	...
	<div class="paging-container">
       <ul>
         <c:if test="${pagination.curRange != 1}">
           <li class="page-num"><a onclick="superAdminMallManger_search('1')"> [처음] </a></li>
         </c:if>
         <c:if test="${pagination.curPage != 1}">
           <li class="page-num"><a onclick="superAdminMallManger_search('${pagination.prevPage}')"> [이전] </a></li>
         </c:if>
         <c:forEach var="pageNum" begin="${pagination.startPage}" end="${pagination.endPage}">
           <c:choose>
             <c:when test="${pageNum == pagination.curPage}">
               <li class="page-num"><a class="page-num-selected" onclick="superAdminMallManger_search('${pageNum}')"> ${pageNum} </a></li>
             </c:when>
             <c:otherwise>
               <li class="page-num"><a onclick="superAdminMallManger_search('${pageNum}')"> ${pageNum} </a></li>
             </c:otherwise>
           </c:choose>
         </c:forEach>
         <c:if test="${pagination.curPage != pagination.pageCnt && pagination.pageCnt > 0}">
           <li class="page-num"><a onclick="superAdminMallManger_search('${pagination.nextPage}')"> [다음] </a></li>
         </c:if>
         <c:if test="${pagination.curRange != pagination.rangeCnt && pagination.rangeCnt > 0}">
           <li class="page-num"><a onclick="superAdminMallManger_search('${pagination.pageCnt}')"> [끝] </a></li>
         </c:if>
       </ul>
     </div>
